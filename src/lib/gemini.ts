...
/**
 * Explain a detected eye disease using Gemini.
 * Formulates a structured prompt and calls Gemini REST API.
 * 
 * @param disease - The name of the predicted disease
 * @param confidence - Confidence score (0 to 100)
 * @param lang - Language code: "en", "hi", or "gu"
 */
export async function explainDisease(
  disease: string,
  confidence: number,
  lang: string,
): Promise<GeminiResponse> {
  const userPrompt = `The RetinaSense-ViT eye screening model has predicted: "${disease}" with a confidence of ${confidence.toFixed(1)}%.
...
Please organize the response with clear headings or bullet points. Include a disclaimer at the end that this is a screening/educational tool and the user should consult an ophthalmologist for a definitive diagnosis.`;

  return askArogyaSathi(userPrompt, lang, []);
}

/**
 * Validate if the uploaded image is actually a retinal fundus photograph.
 * Uses Gemini multimodal vision capabilities to classify the image type.
 * 
 * @param base64DataUrl - The base64 data URL of the image
 */
export async function validateEyeImage(base64DataUrl: string): Promise<"FUNDUS" | "EXTERNAL_EYE" | "INVALID"> {
  if (!API_KEY) {
    console.error("[ArogyaSathi] VITE_GEMINI_API_KEY is not set");
    return "FUNDUS"; // fallback to FUNDUS if no key configured to prevent blockages
  }

  // Extract the mime type and raw base64 data from the data URL
  ...
  const model = "gemini-2.5-flash"; // Use primary multimodal flash model
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${API_KEY}`;
...
1. "FUNDUS": A valid internal retinal fundus photograph. These are circular scans showing the inside of the eye (retina) with an orange, red, or dark brown background, showing fine red blood vessels and the optic disc.
2. "EXTERNAL_EYE": An external photograph of the eye from the outside, showing the iris, pupil, eyelashes, sclera (white part of the eye), or eyelids.
3. "INVALID": Any other image (faces, skin, objects, landscapes, etc.).

Respond with ONLY the category name: "FUNDUS", "EXTERNAL_EYE", or "INVALID". Do not add any punctuation, explanation, or other text.`,
              },
            ],
          },
        ],
    ...
    if (response.ok) {
      ...
      console.log("[ArogyaSathi] Eye image validation result:", text);
      
      if (!text) {
        console.warn("[ArogyaSathi] Gemini image validation returned empty text (likely safety filtered). Falling back to FUNDUS.");
        return "FUNDUS";
      }

      if (text.includes("EXTERNAL") || text.includes("OUTSIDE")) return "EXTERNAL_EYE";
      if (text.includes("FUNDUS") || text.includes("RETINA")) return "FUNDUS";
      return "INVALID";
    }

    console.warn("[ArogyaSathi] Gemini image validation failed:", response.status);
    return "FUNDUS"; // Fallback to FUNDUS so we don't block
  } catch (err) {
    console.error("[ArogyaSathi] Error validating eye image:", err);
    return "FUNDUS"; // Fallback to FUNDUS
  }
}
